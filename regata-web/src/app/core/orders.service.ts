...
export interface CheckoutRequest { items: CheckoutItem[]; discountCode?: string | null; reservationToken?: string | null; }
...
export interface CheckoutResponse { orderId: string; subtotal: number; discount: number; total: number; currency: string; checkoutUrl: string; }
export interface QuoteResponse { subtotal: number; discount: number; total: number; currency: string; items: any[]; }
